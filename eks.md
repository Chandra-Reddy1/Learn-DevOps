...
| **Pod can't resolve DNS**                         | DNS lookup is failing                                                    | `kubectl exec <pod> -- nslookup google.com`                                          | CoreDNS issue, DNS config, network policy                | Check CoreDNS pods/logs and `/etc/resolv.conf`                           |
| **`Could not resolve host`**                      | DNS name cannot be resolved                                              | `nslookup <hostname>` / `dig <hostname>`                                             | CoreDNS, VPC DNS, wrong hostname                         | Fix DNS configuration/CoreDNS                                            |
| **`Connection timed out`**                        | Packets aren't reaching the destination or response is blocked           | `curl`, `nc -vz <host> <port>`                                                       | Security Group, NACL, route, firewall                    | Check routing, SG, NACL, destination firewall                            |
| **`Connection refused`**                          | Destination is reachable but nothing accepts the connection on that port | `nc -vz <host> <port>`                                                               | Application not listening, wrong port                    | Check application, `targetPort`, `containerPort`, Service                |
| **Service has no endpoints**                      | Service isn't finding ready pods                                         | `kubectl get endpoints <svc>` / `kubectl get endpointslices`                         | Wrong selector/labels, pod not Ready                     | Fix Service selector or pod readiness                                    |
| **Service returns 503**                           | No healthy backend is available                                          | `kubectl describe svc <svc>`; check endpoints                                        | No ready pods, wrong selector, backend failure           | Fix pod readiness/selector/backend                                       |
| **ALB target unhealthy**                          | ALB health check fails                                                   | Check target group + `kubectl describe ingress`                                      | Wrong health path/port, SG, readiness, app failure       | Fix health check, ports, SG, application                                 |
| **ALB returns 502**                               | ALB can't successfully communicate with backend                          | Check target health, Service, endpoints                                              | Wrong port, backend connection failure, app issue        | Fix target/service port and backend connectivity                         |
| **ALB returns 503**                               | ALB has no usable healthy targets                                        | Check target group health                                                            | Pods unavailable/unhealthy                               | Fix pod readiness, endpoints, target registration                        |
| **Ingress not getting an ALB**                    | AWS LB Controller isn't creating/managing ALB                            | `kubectl describe ingress <ingress>`; controller logs                                | Controller issue, IAM, subnet tags, IngressClass         | Fix controller/IAM/subnets/Ingress config                                |
| **AWS Load Balancer Controller `AccessDenied`**   | Controller lacks AWS IAM permissions                                     | Controller logs + ServiceAccount                                                     | Incorrect IAM role/policy                                | Fix IAM role/permissions and workload identity                           |
| **Pod IP not assigned**                           | CNI couldn't allocate networking                                         | `kubectl get pods -n kube-system`; `kubectl logs -n kube-system -l k8s-app=aws-node` | IP exhaustion, CNI issue, ENI limit                      | Free/expand subnet IPs, fix CNI, use prefix delegation where appropriate |
| **`FailedCreatePodSandBox`**                      | Kubernetes couldn't create the pod network sandbox                       | `kubectl describe pod <pod>`                                                         | CNI failure, IP exhaustion, networking config            | Check VPC CNI logs, subnet IPs, IAM, node networking                     |
| **`NetworkPluginNotReady`**                       | Node networking isn't ready                                              | `kubectl describe node <node>`                                                       | VPC CNI failure                                          | Check `aws-node` DaemonSet/logs and node networking                      |
| **Pod-to-pod communication fails**                | Pods can't communicate with each other                                   | `kubectl exec`; `ping`/`curl`/`nc`                                                   | NetworkPolicy, CNI, SG for Pods, routing                 | Check NetworkPolicy, CNI, security groups                                |
| **Pod can't reach RDS**                           | Application can't connect to database                                    | `nc -vz <rds-endpoint> 5432`                                                         | RDS SG, route, wrong port, DNS                           | Fix RDS SG/routing/DNS/port                                              |
| **RDS connection timeout**                        | Network path or firewall blocks DB connection                            | `nc -vz <rds-endpoint> 5432`                                                         | Security Group/NACL/route                                | Fix inbound RDS SG and network path                                      |
| **EKS node can't reach AWS service**              | Node can't access AWS endpoint                                           | `curl`, VPC route checks                                                             | NAT/endpoint/route/SG                                    | Configure NAT Gateway or appropriate VPC endpoint                        |
| **Private subnet has no internet**                | Private resources have no outbound path                                  | Check route table                                                                    | Missing NAT Gateway/route                                | Add/configure NAT Gateway and private route                              |
| **NAT Gateway failure**                           | Private subnet outbound connectivity is broken                           | VPC route/NAT metrics                                                                | NAT unavailable, route misconfiguration                  | Fix NAT/route or use appropriate VPC endpoints                           |
| **`i/o timeout`**                                 | Network connection timed out                                             | `curl`, `nc`, route/SG checks                                                        | Firewall, SG, NACL, route                                | Trace the network path                                                   |
| **`No route to host`**                            | Routing/network path doesn't exist                                       | Check route tables                                                                   | Missing route, subnet/VPC routing                        | Correct route table configuration                                        |
| **NodePort unreachable**                          | Node port isn't reachable                                                | `kubectl get svc`; test node IP/port                                                 | SG, NodePort, Service/backend issue                      | Check node SG, Service, endpoints                                        |
| **NetworkPolicy blocks traffic**                  | Kubernetes network policy denies connection                              | `kubectl get networkpolicy -A`                                                       | Incorrect ingress/egress rules                           | Update policy to allow required traffic                                  |
| **Cross-AZ connectivity issue**                   | Workload can't communicate across AZs                                    | Check routes/SG/NACL                                                                 | Network configuration                                    | Correct VPC routing and security rules                                   |
| **Security Group blocks traffic**                 | AWS firewall denies traffic                                              | Check SG inbound/outbound rules                                                      | Missing port/source rule                                 | Allow only required source/port                                          |
| **NACL blocks traffic**                           | Subnet-level stateless ACL denies traffic                                | Check subnet NACL                                                                    | Missing inbound/outbound rule or ephemeral ports         | Correct NACL rules                                                       |
| **Wrong Service `targetPort`**                    | Service forwards to a port the container isn't listening on              | `kubectl describe svc <svc>`                                                         | Port mismatch                                            | Correct `targetPort`                                                     |
| **Wrong `containerPort`**                         | Manifest documents/uses incorrect container port                         | `kubectl describe pod <pod>`                                                         | Port mismatch                                            | Correct container/application port                                       |
| **External DNS works but application doesn't**    | DNS is fine; failure is downstream                                       | `nslookup`; then `curl`                                                              | ALB/backend/Service issue                                | Continue tracing DNS → ALB → Service → Pod                               |
| **Intermittent connection failures**              | Some requests work and others fail                                       | Check ALB targets/endpoints                                                          | One unhealthy pod/node, uneven targets, flaky dependency | Identify failing target and remove/fix it                                |
| **Connection works from one pod but not another** | Network configuration differs by pod/node                                | Test from both pods                                                                  | NetworkPolicy, node routing, SG for Pods                 | Compare node, subnet, policy, and security configuration                 |
